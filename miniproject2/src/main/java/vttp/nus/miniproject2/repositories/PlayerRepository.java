        ...
        try {
            ...
            for (Map<String, Object> playerData : players) {
                ...
                if (playerName.equals(player.getName())) {
                    ...
                    List<Integer> scores = (List<Integer>) playerData.get("score");
                    if (responseStrings == null && scores == null) {
                        responseStrings = new ArrayList<>();
                        scores = new ArrayList<>();
                        playerData.put("responses", responseStrings);
                        playerData.put("score", scores);
                    } else if (responseStrings != null && scores == null) {
                        playerData.put("responses", responseStrings);
                    }

                    responseStrings.add(player.getResponses().get(player.getResponses().size() - 1));
        ...
        if (playersJson != null) {
            try {
                ...
                for (Map<String, Object> playerMap : playersList) {
                    ...
                    List<Integer> scores = (List<Integer>) playerMap.get("score");
                    if (scores != null && !scores.isEmpty()) {
                        player.setScore(scores.get(0));
                    } else {
                        player.setScore(0);
                    }
                    ...
                    if (responseStrings != null) {
                        player.setResponses(responseStrings);
                    }
